...
public partial class HeavyAlienBrainCreator : MonoSingleton<HeavyAlienBrainCreator>
{
    ...
    private void Epoch()
    {
        ...
        float avg = 0.0f;
        foreach (var g in currentPopulation)
            avg += (float)g.fitness;
    ...
    int Selection()
    {
        ...
        foreach (Genome g in currentPopulation)
    ...
    void Crossover(Genome mum, Genome dad, out Genome baby1, out Genome baby2)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < crossPointNumber; ++i)
                crossPoints.Add(UnityEngine.Random.Range(1, mum.weights.Count - 1));
            crossPoints.Add(0);
